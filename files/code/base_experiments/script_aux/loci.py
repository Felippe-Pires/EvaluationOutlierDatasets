...
def run_loci(data: np.ndarray, alpha: float = 0.5, k: int = 3, max_dist: float = None,
    ...
    if method == "distance-matrix":
        loci_i = LOCIMatrix(data, alpha, k, max_dist)
        loci_i.run()
    elif method == "distance-matrix-binary":
        loci_i = LOCIMatrixBinarySearch(data, alpha, k, max_dist)
        loci_i.run()
    else:
...
class LOCI(object):
    ...
    def __init__(self, data: np.ndarray, alpha: float = 0.5, k: int = 3, max_dist=None):
        self._data = data
        self._alpha = alpha
        self._k = k
    ...
    def run(self):
        ...
        for p_ix in range(self._n_points):
            ...
            for r in critical_values[len(critical_values)-10:]:
...
class LOCIMatrix(LOCI):
    ...
    def __init__(self, data: np.ndarray, alpha: float = 0.5, k: int = 3, max_dist: float = None):
        super().__init__(data, alpha, k, max_dist)
