...
def graph_ranks(avranks, names, p_values, cd=None, cdmethod=None, lowv=None, highv=None,
    ...
    height = (cline + ((k + 1) / 2) * 0.2 + minnotsignificant) + 0.5
    ...
    for clq in cliques:
        ...
        line([(rankpos(ssums[min_idx]) - side, start),
              (rankpos(ssums[max_idx]) + side, start)], color='brown',
             linewidth=linewidth_sign)
        start += height
...
def draw_cd_diagram(df_perf=None, alpha=0.05, title=None, labels=False, output='cd-diagram.png'):
    """
...
def wilcoxon_holm(alpha=0.01, df_perf=None):
    """
